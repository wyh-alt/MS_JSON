"""本地版音频导出页面：从母文件夹的 MSID 子文件夹中读取本地音频代替直链下载。"""
import os
...
class LocalAudioWorker(QThread):
...
class LocalAudioDownloadPage(ScrollArea):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setObjectName("localAudioDownloadPage")
        ...
        layout.addWidget(TitleLabel("音频资源导出（本地版）"))
        layout.addWidget(
            BodyLabel(
                "从母文件夹的各 MSID 子文件夹中读取本地音频资源，"
                "支持单轨导出与合并伴奏混音，并按指定格式转码输出。"
            )
        )
        ...
        self.loudness_spin = create_signed_value_input(
            output_option_card, minimum=-48, maximum=-1, value=-12, suffix="LUFS",
        )
        ...
        self.track_gain_spin = create_signed_value_input(
            output_option_card, minimum=-60, maximum=10, value=-6, suffix="dB",
        )
        ...
        self.limiter_spin = create_signed_value_input(
            output_option_card, minimum=-24, maximum=-1, value=-1, suffix="dBTP",
        )
        ...
        self.worker: LocalAudioWorker | None = None
    ...
    def _start_export(self):
        ...
        params = dict(parent_dir=parent_dir, output_dir=output_dir, options=options)
        if self._path_loader is not None and self._path_loader.isRunning():
    ...
    def _on_finished(self, result: LocalAudioResult):
