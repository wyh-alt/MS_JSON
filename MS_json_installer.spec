...
# onedir 模式 spec：构建 dist\MS_json\ 目录（exe + _internal 全部依赖），
# 供 Inno Setup 打包成安装程序，安装后所有组件直接解压到安装目录。
...
exe = EXE(
    ...
    icon="icon.ico",
    # onedir 下把依赖全部放到与 exe 同级的目录（不建 _internal 子目录），
    # 安装目录即程序目录，ffmpeg.exe、icon.png 与 MS_json.exe 同级，
    # 运行时路径解析（find_ffmpeg_executable、_app_dir）直接命中。
    contents_directory=".",
)
