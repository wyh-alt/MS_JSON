...
| | `file_mr_drum` | 鼓轨（不区分性别，仅此一个字段指向文件） |
...
勾选需要处理的内容（伴奏处理 / MIDI处理 / 歌词处理 / 段落信息导出 / 交付总表导出，默认全选），输入 JSON 文件夹后一键提取，全部使用各模块默认参数：
...
按歌曲生命周期处理：每首歌固定按 **伴奏 → MIDI → 歌词 → 段落** 顺序串行执行勾选项目，
歌曲之间并行（默认 3 首并发），交付总表作为整体步骤与歌曲循环并行。
同一首歌的各项目共享一次音频校准（校准音频优先复用缓存，未命中才重新下载），
结果保证一致；单曲某项目失败不影响该曲其他项目与其他曲目。
全部结束后弹出处理日志框，逐项目显示成功 / 校准说明 / 失败明细与输出文件，可一键导出日志文本。
...
**有效旋律判定**：过滤 rap 与非旋律音符后无音符（整首 rap 或疑似无实际音高）的歌曲
没有旋律可对齐，MIDI 不会产出，歌词 / 段落 / MIDI 所有模块一致地跳过校准（该曲按
无校准导出，日志注明原因）。

**分析窗动态扩展**：默认分析前 30s，若窗口内无能量（长静音前奏的旋律音频，如起音在
45s 的歌曲），自动扩展窗口（30s → 60s → 120s → 240s）直到检测到能量，找到真实起音
再与 MIDI 音符匹配；全部窗口均无能量才判定为异常静音并跳过校准。

**异常旋律音频检测**：能量包络几乎恒定的音频（单一频段 / 无实际内容，如整段恒定音）
会被起音检测误判为"音频从 0ms 开始"而产生错误偏移。分析时若帧 RMS 变异系数 < 0.05
或 RMS 波峰因数 < 2.0（正常旋律实测 0.36~1.78 / 5~11），判定为异常旋律音频，跳过
该校准（该曲按无校准导出，日志注明原因）。

MIDI 导出、歌词导出、段落导出均可独立开关「音频参考校准」。

**共享校准**：同一首歌（同一旋律音频、同一 JSON 缓存目录）的校准结果按歌曲缓存，
第一次计算后由各模块复用，避免各模块独立计算导致"一个带校准、一个没带"的不一致。
成功与失败结果都缓存；音频文件状态变化（下载完成/被替换）后自动重算，失败结果
5 分钟内复用、之后允许重试恢复。
...
- 中文子文件夹：`专辑封面`、`男调旋律`、`女调旋律`、`男调伴奏`、`女调伴奏`、`鼓轨`。
...
**职责**：按歌曲生命周期执行伴奏 / MIDI / 歌词 / 段落 / 交付总表导出，统一进度聚合与结果收集。

**并行编排**

- `run_delivery_export()` 按歌曲工作流编排：每首歌固定按 伴奏 → MIDI → 歌词 → 段落 串行执行勾选项目（`_SONG_PROJECT_ORDER`），歌曲之间并行（`ThreadPoolExecutor`，歌曲级并发默认 3），交付总表作为整体步骤与歌曲循环并行（不占歌曲并发槽位）。
- 同一首歌的各项目共享一次音频校准（`core.audio_calibration` 的按歌曲校准缓存），结果一致；段落信息逐曲收集到共享列表，全部歌曲完成后统一写入 `歌词段落信息及时间点.xlsx`。
- 各项目进度经 `_ProgressAggregator`（歌曲 worker 每完成一个项目步骤 `advance()` 一次）加权聚合为单一进度（百分比 + `[项目名] 状态` 文案），线程安全；各项目结果由线程安全的 `_ProjectCollector` 聚合。
- 单曲某项目失败记录到该项目并继续该曲下一项目，不影响其他曲目；JSON 解析失败则该曲全部启用项目记录失败。
